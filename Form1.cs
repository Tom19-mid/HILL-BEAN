...
namespace HILL_BEAN
{
    public partial class Form1: Form
    {
        public Form1()
        {
            ...
            Image flippedImageHorizontal = FlipImage(originalImage, true); // true cho chiều ngang

            // Lật ảnh theo chiều dọc
            Image flippedImageVertical = FlipImage(originalImage, false); // false cho chiều dọc

            // Hiển thị ảnh đã lật vào PictureBox
            PtbhoaCoffe2.Image = flippedImageHorizontal; // Hiển thị ảnh đã lật theo chiều ngang
            // pictureBox1.Image = flippedImageVertical; // Nếu bạn muốn lật theo chiều dọc, bỏ comment dòng này

        }

        private Image FlipImage(Image originalImage, bool flipHorizontal)
        {
             // Tạo một bitmap mới với kích thước giống ảnh gốc
            Bitmap flippedImage = new Bitmap(originalImage);
            
            // Sử dụng Graphics để lật ảnh
        ...
        private void MakeRoundPtbIconCoffee(PictureBox ptbIconCoffee, int raidus)
        {
            ...
            PtbconCoffee.Width = 100;  // Đặt chiều rộng là 200
            PtbconCoffee.Height = 100; // Đặt chiều cao là 200
